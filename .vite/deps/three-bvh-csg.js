...
var HalfEdgeMap = class {
  ...
  updateFrom(geometry) {
    ...
    function hashPositionAttribute(i) {
      _vec32.fromBufferAttribute(posAttr, i);
      return hashVertex3(_vec32);
    }
      ...
      for (let k = 0, l = attrKeys.length; k < l; k++) {
        ...
        switch (attr.itemSize) {
          case 1:
            ...
            break;
          case 2:
            str = hashVertex2(_vec22.fromBufferAttribute(attr, i));
            break;
          case 3:
            str = hashVertex3(_vec32.fromBufferAttribute(attr, i));
            break;
          case 4:
            str = hashVertex4(_vec42.fromBufferAttribute(attr, i));
            break;
        }
        if (result !== "") {
          result += "|";
...
function appendAttributeFromIndex(index, attributes, matrixWorld, normalMatrix, attributeData, invert = false) {
  for (const key in attributeData) {
    ...
    const itemSize = attr.itemSize;
    if (key === "position") {
      _vec33.fromBufferAttribute(attr, index).applyMatrix4(matrixWorld);
      arr.push(_vec33.x, _vec33.y, _vec33.z);
    } else if (key === "normal") {
      _vec33.fromBufferAttribute(attr, index).applyNormalMatrix(normalMatrix);
      if (invert) {
        _vec33.multiplyScalar(-1);
      }
      arr.push(_vec33.x, _vec33.y, _vec33.z);
    } else {
